...
![NER 未解难点的分类对照](/posts/kg/04-ner-open-problems.webp "1600x726")
...
上表列出的这些方向里**没有「不连续实体」**，两个年份版本也都没有出现生成式大模型做 NER 的内容，方法线停在了「预训练编码器 + 跨度分类 / 序列标注」。所以「到大模型」这一半，材料的覆盖范围到此为止：本篇不对 LLM 生成式抽取做事实性陈述。
