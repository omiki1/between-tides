...
`scripts/check-markdown-residue.mjs` 校验文章渲染后有没有 markdown 残留，当前主要针对加粗失效。

### 加粗失效这件事值得单独记一笔

页面上曾出现字面的 `**`。实测确定的失败条件是：**标点紧接 `**`，且 `**` 后面还有非空白字符**。

| 写法 | 结果 |
| :--- | :--- |
| `**概念图（Conceptual Graph）**由 John 提出` | ❌ 残留 `**` |
| `**把表示做对。**Zhou 2016 的……` | ❌ 残留 `**` |
| `**概念图（Conceptual Graph）** 由 John 提出` | ✅ 正常 |
| `**概念图（Conceptual Graph）**`（行尾） | ✅ 正常 |

原因是 CommonMark 规定：作为**结束**分隔符的 `**`，其左侧不能是空白或数字。中文标点（`）`、`。`、`，`……）与半角 `)` 都不被识别为「标点」，于是这个 `**` 既不是「左侧是标点」也不是「左侧是空白」，无法作为结束符。

修法是在 `**` **之后**补一个空格。**空格补在之前会让问题更严重** —— 那正好违反「左侧不能是空白」，开发中搞反过一次，把 106 处改成了必然失败的形态。

因此校验脚本采用**渲染验证**而不是源码正则：`**` 既是开始符也是结束符，同段文本里还可能出现连续的结束符（`**A**内容**B**内容`）以及代码块里的字面 `**`（Python 的 `dim ** 0.5`），靠「按出现顺序奇偶配对」判定会误伤开始符 —— 实际发生过，一次误伤 88 处。

脚本的做法是：每试一处改动就真的调用 `lib/markdown.ts` 的 `renderMarkdown`，只有让残留数下降的改动才保留；写回后再读一次验证。**渲染器怎么认为，校验就怎么认为。** 代码块内的星号属正常内容，比较时会先剔除 `<pre>` 与 `<code>`。

### 必须同时检查字面 `**` 与孤立 `*`

只查 `**` 是不够的。源文件里的 `**` 中间一旦混进空格，会变成 `* *`：

```
源码：**实体识别（Named Entity Recognition，NER）* *要抽的是
渲染：<p>*<em>实体识别（Named Entity Recognition，NER）</em> *要抽的是
```

渲染结果里**一个 `**` 都没有**，只有两个孤立 `*`，但页面明显是坏的。这个形态曾让校验错误地通过，因此现在两类都查：
- 字面 `**` —— 加粗完全没解析
- 孤立 `*` —— 加粗被撕开

修复分三步：① 先把 `* *` 无条件归位成 `**`（这个写法无歧义，正文里不会合法出现）；② 再对「标点 + `**` + 非空白」在 `**` 之后补空格；③ 写回后重新渲染验证，不干净就整个放弃。

第 ① 步必须先做且不做比较：归位后的 `**` 往往又落入「后跟正文」的失败形态，需要接着补空格。若把两类改动放在同一个循环里逐次比残留数，就会在归位这一步卡住不动。
